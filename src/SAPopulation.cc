...
SAPopulation::SAPopulation(SAConfig *t_config, const ParametersRange& t_par_range) : m_config(t_config), m_mt(std::random_device()()), m_par_range(t_par_range) {
...
void SAPopulation::moveParticles(std::function<double (double*)> cost_function, int iter)
{
  if (iter > 0) {
    ...
    double Tinit = m_config->getTemp();

    m_config->setTemp(Tinit / log(1 + iter));

    std::uniform_real_distribution<double> uni(0, 1);
    for (size_t i = 0; i < m_particles[i].getDimension(); ++i) {
      Particle guess(m_particles[i].getDimension());
      for (size_t j = 0; j < m_particles[i].getDimension(); ++j) {
