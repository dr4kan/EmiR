...
PS_algorithm::PS_algorithm(Function obj_function, S4 config) :
Algorithm(obj_function),
m_population(PSPopulation(obj_function)) {
  ...
  m_algo_config.setCognitiveParameter(config.slot("cognitive"));
  m_algo_config.setSocialParameter(config.slot("social"));
  m_algo_config.setVmaxParameter(config.slot("velocity_max"));
  m_algo_config.setInertiaParameter(config.slot("inertia"));
};
//_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/


void PS_algorithm::addPopulationPosition() {
  std::vector<double> v;
...
void PS_algorithm::minimize() {
  ...
  m_cost_history.push_back(m_population.getBestSolution().getBestCostParticle());
  ...
  for (m_iter = 1; m_iter < n_iter; ++m_iter) {
    ...
    // Move particles, evaluate and sort
    m_population.moveParticles();
...
S4 PS_algorithm::getResults() {
  S4 result("MinimizationResult");
  result.slot("algorithm")       = "PS";
  result.slot("iterations")      = m_iter;
  result.slot("obj_function")    = m_obj_function;
  result.slot("best_cost")       = m_population.getBestSolution().getBestCostParticle();
  result.slot("best_parameters") = m_population.getBestSolution().getBestPositionParticle();
