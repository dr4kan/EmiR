...
#' @param freq_min ???
#' @param freq_max ???
#' @return `config_new` returns an object of class `ABCConfig`.
#' @export
config_new <- function(iterations,
                       population_size,
...
#' used by algorithm (see \link[EmiR]{config_new}).
#' @param ... additional options (see \link[EmiR]{MinimizerOpts}).
#' @return `minimize_new` returns an object of class `MinimizationResult`.
#' @importFrom Rdpack reprompt
#' @references \insertRef{yang2010new}{EmiR}
#' @export
minimize_new <- function(obj_func, parameters, config, constraints = NULL, ...) {
  ...
  tictoc::tic()
  out <- cpp_minimize_new(obj_func, constraints, parameters, config, opt)
