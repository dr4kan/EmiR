...
#' functions to work. Default is `FALSE`.
...
#' by adding a term, called \emph{penalty function}, to the objective function. The penalty function
...
#' positive (or negative in case of maximization) number if any constraint is violated;
#' * `"ACCREJ"` - Acceptance-Rejection method: a solution violating any constraint is replaced by a
...
#'
#' Default is `"DIS"`. For constrined optimization problems only `"DIS"` is supported.
